resource "aws_ecs_task_definition" "demo" {
  faimly                   = "demo"
  required_campatibilities = ["FARGATE"] # Matlab:ECS task Fargate par chalega.
  # hume EC2 server manage nahi karna padega.

  network_model = "awsvpc" #Fargate ke liye important hai. Task ko apna network interface/IP milta hai.


  cpu    = "256" #Initial demo ke liye small allocation.
  memory = "512"

  execution_role_arn = aws_iam_role.ecs_task_definition_role.arn #Terraform automatically dependency samajh lega:
  #Yani IAM role pehle create hoga.

  container_definition = jsonencode([
    {
      name  = "demo"
      image = "${aws_ecr_repository.demo.repoisoty_url}:latest" 
